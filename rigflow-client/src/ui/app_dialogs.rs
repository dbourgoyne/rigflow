...
impl RigflowApp {
    pub(crate) fn draw_add_bookmark_dialog(&mut self, ctx: &egui::Context) {
                ...
                .show(ctx, |ui| {
                    ...
                    if let Ok(mut state) = self.state.lock() {
                        ...
                        if !state.bookmark_status.is_empty() {
                            ui.add_space(8.0);
                            ui.colored_label(egui::Color32::RED, &state.bookmark_status);
                        }
    ...
    pub(crate) fn draw_add_operator_dialog(&mut self, ctx: &egui::Context) {
                ...
                .show(ctx, |ui| {
                    ...
                    if let Ok(mut state) = self.state.lock() {
                        ...
                        if !state.persistence_status.is_empty() {
                            ui.add_space(8.0);
                            ui.colored_label(egui::Color32::RED, &state.persistence_status);
                        }
