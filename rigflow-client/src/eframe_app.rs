...
use crate::app::layout::{LEFT_GUTTER, RIGHT_GUTTER, WATERFALL_IMAGE_HEIGHT, WATERFALL_IMAGE_WIDTH};
                            ...
                            if let Some(tex) = &self.waterfall_texture {
                                ...
                                ui.horizontal(|ui| {
                                    ...
                                    if response.clicked() && snapshot.input_sample_rate_hz > 0.0 {
                                        if let Some(pointer_pos) = response.interact_pointer_pos() {
                                            let frac = ((pointer_pos.x - response.rect.left())
                                                / response.rect.width())
                                                .clamp(0.0, 1.0);
                                            ...
                                            clicked_freq_hz =
                                                Some(x_frac_to_frequency_hz(frac, &state_snapshot));
                                        }
                                    }
                                });
                                ...
                                if let Some(clicked_freq_hz) = clicked_freq_hz {
                                    if !snapshot.radio_acquired {
                                        if let Ok(mut state) = self.state.lock() {
                                            state.server_status =
                                                "cannot tune: no radio acquired".to_string();
                                        }
                                    } else {
                                        if let Ok(mut state) = self.state.lock() {
