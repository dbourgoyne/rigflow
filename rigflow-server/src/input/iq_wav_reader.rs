...
/// IQ WAV reader that supports:
/// - stereo IQ (I = left, Q = right)
/// - i16 and f32 sample formats
///
/// Not real-time: reads as fast as requested.
...
impl IqWavReader {
    ...
    ///
    /// Dispatches to the correct format-specific reader.
    ...
    fn read_block_i16(&mut self, max_complex_samples: usize) -> Result<Vec<Complex32>, String> {
        ...
        // Important: must reuse iterator, not recreate per loop
        let mut samples = self.reader.samples::<i16>();

        for _ in 0..max_complex_samples {
            let (i, q) = match (samples.next(), samples.next()) {
                (Some(Ok(i)), Some(Ok(q))) => (i, q),
                (Some(Err(e)), _) => return Err(format!("error reading I sample: {e}")),
                (_, Some(Err(e))) => return Err(format!("error reading Q sample: {e}")),
                _ => break, // EOF
            };
